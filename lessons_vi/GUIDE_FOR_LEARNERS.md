...
Sau 21 bài học, bạn sẽ hiểu cách một hệ thống tạo nội dung SEO bằng AI hoạt động — từ khâu nghiên cứu chủ đề đến khi tạo ra một bài viết hoàn chỉnh có hình ảnh. Bạn sẽ có khả năng chạy, chỉnh sửa, giải thích từng thành phần, và sử dụng các công cụ AI như Claude Code để mở rộng hệ thống.
...
| Node.js 18+ | Chạy giao diện frontend | `node --version` trong terminal |
...
| `DATA_FOR_SEO_API_KEY` | Tuỳ chọn (tìm kiếm web, hình ảnh, AIO) | Có bản dùng thử miễn phí |
...
Mở Bài 04 (`lessons_vi/01-lap-trinh-co-ban/04_thiet_lap_va_package.ipynb`) và chạy cell kiểm tra ở cuối bài. Cả 3 mục đều phải pass.
...
Bắt đầu với `01-lap-trinh-co-ban/01_hello_python.ipynb`.
...
### Mô-đun 3: Xây dựng Agent (bài 8-13)
...
| 11 | Gọi API (cách agent giao tiếp với Claude) | 30 phút |
| 12 | Nối chuỗi agent thành pipeline | 30 phút |
| 13 | Xây mini pipeline (cầu nối sang production) | 50 phút |

Sau Mô-đun 3 bạn có thể: tạo agent AI, trang bị tools, bắt agent trả dữ liệu có cấu trúc, nối chuỗi chúng lại, và xây dựng một mini pipeline hoàn chỉnh.

### Mô-đun 4: Xây dựng sản phẩm (bài 14-17)

Bài 14 (Claude Code) không cần API key. Bài 15-17 cần `ANTHROPIC_API_KEY`.

| Bài học | Chủ đề | Thời lượng |
|---------|--------|------------|
| 14 | Cơ bản về Claude Code (cài đặt, CLAUDE.md, quy trình làm việc) | 45 phút |
| 15 | Agent Content Writer | 45 phút |
| 16 | Agent Image Finder + AIO Analyzer | 45 phút |
| 17 | Team và xử lý hàng loạt (batch) | 30 phút |

Sau Mô-đun 4 bạn có thể: dùng Claude Code để điều hướng codebase, hiểu từng agent, và chạy tạo bài viết hàng loạt.

### Mô-đun 5: Sản phẩm hoàn chỉnh (bài 18-21)

| Bài học | Chủ đề | Thời lượng |
|---------|--------|------------|
| 18 | Lưu trữ file cục bộ (local file storage) | 45 phút |
| 19 | Cách mọi thứ kết nối | 20 phút |
| 20 | Giao diện web (AgentOS + React) | 30 phút |
| 21 | Mở rộng sản phẩm | 50 phút |

Sau Mô-đun 5 bạn có thể: sử dụng sản phẩm hoàn chỉnh qua giao diện web, hiểu cách các thành phần kết nối với nhau, và dùng Claude Code để mở rộng và chỉnh sửa sản phẩm.
...
- **Markdown** — Định dạng văn bản cho tài liệu có format. Các bài viết được viết bằng Markdown:
  ```markdown
...
- **LLM (Large Language Model)** — Mô hình ngôn ngữ lớn. "Bộ não" AI dự đoán văn bản. Claude và GPT đều là LLM.
- **Token** — Đơn vị xử lý của LLM (~3/4 từ tiếng Anh). Chi phí API được tính theo token.
...
- **Model** — Bộ não AI. Hệ thống dùng Claude Sonnet cho tất cả agent (nhanh, tốt với tools, viết tốt).
- **Instructions (Chỉ dẫn)** — Các hướng dẫn định hình cách agent hoạt động. Giống như mô tả công việc.
- **Tools (Công cụ)** — Khả năng bạn trao cho agent (tìm kiếm web, tìm ảnh, lưu bài viết). Agent tự quyết khi nào sử dụng.
- **Structured output / output_schema** — Buộc agent trả dữ liệu theo định dạng cụ thể (không phải văn bản tự do).
- **Team (Đội nhóm)** — Nhiều agent phối hợp dưới sự điều phối của một leader phân công nhiệm vụ.
- **AgentOS** — Công cụ của Agno giúp đóng gói Team thành web API tự động.
...
- **Vibecoding** — Dùng công cụ AI như Claude Code để tạo code bằng cách mô tả thứ bạn muốn.
...
- **AIO (AI Overview)** — Câu trả lời do AI tạo ra hiển thị ở đầu kết quả tìm kiếm Google.
...
2. **Đọc thông báo lỗi** trước khi nhờ trợ giúp. Lỗi Python thường nói chính xác vấn đề là gì.
3. **Thoải mái thử nghiệm ở Mô-đun 1-2**. Thay đổi giá trị, phá vỡ mọi thứ, xem chuyện gì xảy ra. Không tốn tiền API.
4. **Cẩn thận chi phí API ở Mô-đun 3-5**. Mỗi lệnh `agent.run()` đều tốn tiền. Đừng chạy cell trong vòng lặp.
5. **Đừng học thuộc cú pháp**. Tập trung hiểu *mỗi thành phần làm gì*.
6. **Mô-đun 2 là lý thuyết nhưng cực kỳ quan trọng**. Nó giải thích *vì sao* hệ thống được thiết kế như vậy. Đừng bỏ qua.

## Sau khi hoàn thành tất cả 21 bài học

Giờ bạn đã hiểu toàn bộ hệ thống. Dưới đây là cách các bài học liên kết với các file production:

| Bài học | Xây dựng hướng tới | File production |
|---------|---------------------|-----------------|
| 05-07 | Hiểu LLM, prompt, lựa chọn model | (Định hình mọi quyết định thiết kế) |
| 08-09 | Tạo agent, tools | `output/backend/agents/content_writer.py` |
| 10 | Pydantic schema | Dùng xuyên suốt các agent |
| 11-13 | Gọi API, nối chuỗi, mini pipeline | `output/backend/agents/team.py` |
| 15 | Agent Content Writer | `output/backend/agents/content_writer.py` |
| 17 | Team và xử lý hàng loạt | `output/backend/agents/team.py` |
| 18 | Lưu trữ file cục bộ | `output/backend/tools/storage.py` |
| 19 | Cách mọi thứ kết nối | Tất cả file trong `output/backend/` |
| 20 | Giao diện web | `output/backend/serve.py`, `output/frontend/` |
| 21 | Mở rộng sản phẩm | `CLAUDE.md` (bản thiết kế cho Claude Code) |

Để bắt đầu sử dụng sản phẩm:

```bash
# Khởi chạy ứng dụng web
python output/backend/serve.py        # Terminal 1: backend trên port 7777
cd output/frontend && npm run dev     # Terminal 2: frontend trên port 5173
```
...
| Cell chạy mãi không dừng (>2 phút) | Agent có thể đang chờ API. Nhấn nút dừng (biểu tượng hình vuông) và thử lại. |
