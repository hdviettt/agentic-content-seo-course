...
Hỏi giảng viên để lấy API key nếu bạn chưa có. Để lấy Airtable token, truy cập [airtable.com/create/tokens](https://airtable.com/create/tokens).

### Bước 3: Thiết lập Airtable

Airtable được dùng để lưu trữ bài viết và theo dõi trạng thái. Chạy lệnh này một lần để tạo các bảng:

```bash
python output/tools/airtable.py
```

Thêm `AIRTABLE_BASE_ID` được in ra vào file `.env` của bạn.
...
| 08-09 | Tạo agent, tools | `output/agents/researcher.py` |
| 10, 13 | Pydantic schema | `output/agents/schemas.py` |
| 11-12, 13-15 | Nối chuỗi agent, pipeline | `output/pipeline.py` |
| 14 | Writer + image agent | `output/agents/writer.py`, `output/agents/image.py` |
| 16 | Tầng database Airtable | `output/tools/airtable.py` |
| 17 | Cách mọi thứ kết nối | Tất cả file trong `output/` |
| 18 | Giao diện chat, workspace tools | `output/agents/team.py`, `output/tools/workspace.py` |
...
| `Error: status -> error` khi tạo bài viết | Hỏi giao diện chat để kiểm tra trạng thái bài viết và xem thông báo lỗi. Thường do vấn đề API key. |
